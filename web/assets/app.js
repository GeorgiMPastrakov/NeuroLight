...
let interval = 1000
function size(){const panel=document.getElementById('panel');c.width=window.innerWidth-(panel?panel.offsetWidth:0);c.height=window.innerHeight}
...
function drawLightBox(x,y,st){ctx.fillStyle='#0f131c';ctx.fillRect(x-18,y-42,36,84);const r=12;ctx.fillStyle=st.r?st.red:'#2a3040';ctx.beginPath();ctx.arc(x,y-24,r,0,Math.PI*2);ctx.fill();ctx.fillStyle=st.y?st.yellowC:'#2a3040';ctx.beginPath();ctx.arc(x,y,r,0,Math.PI*2);ctx.fill();ctx.fillStyle=st.g?st.green:'#2a3040';ctx.beginPath();ctx.arc(x,y+24,r,0,Math.PI*2);ctx.fill()}
...
const met=await get('/metrics');metrics=met;document.getElementById('t').textContent=met.t;document.getElementById('avg').textContent=Number(met.avg_wait_proxy||0).toFixed(2);document.getElementById('sv').textContent=met.served_v;document.getElementById('sp').textContent=met.served_p;document.getElementById('sw').textContent=met.switches;document.getElementById('ra').textContent=Number(met.reward_avg||0).toFixed(3);draw()}
